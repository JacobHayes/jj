...
pub fn add_commit(
    ...
    let head_tree_editor = get_head_tree(repo).map(|tree| tree.edit().unwrap());
    let empty_tree_editor_fn = || {
        repo.edit_tree(gix::ObjectId::empty_tree(repo.object_hash()))
            .unwrap()
    };

    let mut tree_editor = parent_tree_editor
        .or(head_tree_editor)
        .unwrap_or_else(empty_tree_editor_fn);
    tree_editor
        .upsert(filename, gix::object::tree::EntryKind::Blob, blob_oid)
        .unwrap();
    let tree_id = tree_editor.write().unwrap().detach();
    let commit_id = commit_tree(repo, reference, tree_id, message, parents);
...
#[allow(unused)]
pub fn set_head_to_name(repo: &gix::Repository, target: &str) {
...
pub fn get_head_tree(repo: &gix::Repository) -> Option<gix::Tree<'_>> {
    ...
    // in fresh repos, there is no tree

    if let Err(gix::reference::head_tree::Error::HeadCommit(
        gix::reference::head_commit::Error::PeelToCommit(peel::to_commit::Error::PeelToObject(
            peel::to_object::Error::Unborn { .. },
        )),
    )) = head_tree
    {
        return None;
