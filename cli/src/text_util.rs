...
pub fn write_truncated_start(
    ...
    let (start, truncated_width) = if data_width > max_width {
        truncate_start_pos_bytes(data, max_width.saturating_sub(ellipsis_width))
    } else {
        truncate_start_pos_bytes(data, max_width)
    };
    ...
    if data_width > max_width {
        ...
        let (start, _) = truncate_start_pos_bytes(ellipsis_data, max_width);
...
pub fn write_truncated_end(
    ...
    let (truncated_end, truncated_width) = if data_width > max_width {
        truncate_end_pos_bytes(data, max_width.saturating_sub(ellipsis_width))
    } else {
        truncate_end_pos_bytes(data, max_width)
    };
    ...
    if data_width > max_width {
        // The ellipsis itself may be larger than max_width, so maybe truncate it too.
        let (truncated_end, _) = truncate_end_pos_bytes(ellipsis_data, max_width);
